...
let list_layout (pair: widget -> widget -> widget)
  ...
  List.fold_right (fun w acc -> (pair w acc)) ws (space (0,0))
...
(*need to create a record thats a list*)
type 'a controller = {
...
let make_controller (v: 'a) : 'a value_controller = 
  let controllers = {change_listeners = []; value = v} in
  {
    add_change_listener = 
      (fun (f: ('a -> unit)) ->
        controllers.change_listeners <- f::controllers.change_listeners);
    
    get_value = (fun () -> (controllers.value));
    
    change_value = 
      (fun (x: 'a) -> 
        (*update the value*)
        controllers.value <- x;
        (*need to iterate thru list and call each element on the value*)
        List.iter (fun (f: ('a -> unit)) -> f x) controllers.change_listeners)
  }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  (*creating our label*)
  let w_b, l_b, n_b = button s in 
  
  (*need to instantiate our value controller*)
  let check_state = make_controller init in 

  (*function to switch the state of the checkbox*)
  let check_change () = update_value (check_state) (fun x -> not x) in 

  (*making our rectange widget*)
  let paint_box (g: Gctx.gctx) = 
    let g_new = Gctx.with_color g (if check_state.get_value() then Gctx.black 
    else Gctx.white) in 
    Gctx.fill_rect g_new (0, 0) (19, 19) in 
  
  (*creating the canvas*)
  let (w_c, n_c) = canvas (20, 20) paint_box in 

  (*adding our event listeners for the box and button*)

  n_c.add_event_listener (
    mouseclick_listener (
      fun () -> check_change();
  ));

  n_b.add_event_listener (
    mouseclick_listener (
      fun () -> check_change();
  ));

  (*creating our hpair*) 
  let comb_w = hlist [w_c; space (0,0); w_b] in 
...
(*making a function that makes 1 radio box*)
...
let radbox (controller: 'a value_controller) (selection: ('a * string)) : 
widget =
  let intended_mode, s = selection in 
  (*creating our label*)
  let w_b, l_b, n_b = button s in 


  (*function to switch the state of the checkbox*)
  let check_change () = controller.change_value intended_mode in 

  (*making our rectange widget*)
  let paint_box (g: Gctx.gctx) = 
    let g_new = Gctx.with_color g (if controller.get_value() = intended_mode 
    then Gctx.black else Gctx.white) in 
    Gctx.fill_rect g_new (0, 0) (10, 20) in 
  
  (*creating the canvas*)
  let (w_c, n_c) = canvas (10, 20) paint_box in 
  ...
  (*creating our hpair*) 
  let comb_w = hlist [w_c; space (10,10); w_b; space (10,10)] in 

  comb_w

let radio_group (modes: ('a * string) list ) (init_selection: 'a) : 
  widget list * 'a value_controller = 
    (*need to create a radio box for every linestartmode, pointmode, and 
    ellipsemode*)
    let controller = make_controller init_selection in 
    (*comblist is (comb_w, n_c, check_state) list*)
    let comb_list = List.map (radbox controller) modes in 
    
    (comb_list, controller)
