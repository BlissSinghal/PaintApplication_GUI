...
(** A graphics context represents a region of the window to which
   ...
   structure -- that is, the operations here use a given graphics
   context to create a new one with the specified
   characteristics. They do not modify their arguments. *)
...
type gctx = {
  ...
  line_width: int;  (*the line thickness*)
  }

(* Internal helper to set the text size -- no need to understand how
   it works *)
...
(*produce new gctx.t with different line_thickness*)
let with_width (g: gctx) (w: int) : gctx = 
  { g with line_width = w}
...
(* A main purpose of the graphics context is to provide a mapping between
...
let draw_string (g: gctx) (p: position) (s: string) : unit =
  ...
  (* We need a little "fudge factor" because font rendering in Web
     browsers requires a little bounding box adjustment *)
  let fudge = if Graphics.js_mode then 3 else 0 in
  (* (We subtract here because we are working with Ocaml coordinates) *)
...
(* This part of the module adapts OCaml's native event handling in the
   Graphics library to something that more closely resembles that
   found in Java. *)
